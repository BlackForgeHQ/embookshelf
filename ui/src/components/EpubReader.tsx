...
type Props = {
  ...
  onReady?: (meta: { toc: EpubTocEntry[]; totalPages: number }) => void;
...
export const EpubReader = forwardRef<EpubReaderHandle, Props>(function EpubReader(
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const toc: EpubTocEntry[] = book.navigation?.toc?.map(mapToc) ?? [];
    ...
    for (const h of list) {
      try {
        r.annotations.add(
          ...
          undefined,
          'hl',
          { fill: h.color ?? 'oklch(0.92 0.07 85)', 'fill-opacity': '0.5', 'mix-blend-mode': 'multiply' },
        );
  ...
  return (
    <div
      ref={containerRef}
      style={{ width: '100%', height: '100%', background: 'var(--color-paper-0)' }}
    />
  );
});
...
function mapToc(node: any): EpubTocEntry {
  return {
    ...
    subitems: Array.isArray(node.subitems) ? node.subitems.map(mapToc) : undefined,
  };
}
