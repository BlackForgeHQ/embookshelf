...
// resolve the library's handle, walk it, hand the listing to
// scan.Reconcile, then touch the library's scan timestamp and log what
// happened. The policy — what a walk does to the files table, and the
// promise that it never creates a row — is scan.Reconcile's, where it
// can be tested without a database.
//
// What is decided here and not there is what to do about a walk that did
// not happen: an unconfigured local library is a state to report, not a
// job to retry, and it must never fall through to a reconcile with an
// empty listing, which would flag every row in the library missing.
//
...
func LibraryScan(ctx context.Context, args jobs.LibraryScanArgs, deps LibraryScanDeps) error {
	...
	// back is library-relative, the same shape the files rows are stored
	// in, so the reconcile is comparing like with like.
	...
	// view of the files table it needs. It reads the table, diffs, and
	// acts; what comes back is counts, because by then it has acted.
