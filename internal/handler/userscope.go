...
// userID == "" { return }` — was restated at roughly two dozen call sites
// across nine files, the same drift class bookScoped closed off for the
// book axis (#241). The failure mode here is milder: auth middleware sits
// upstream of every route, so a forgotten check nil-derefs on a missing
// user rather than leaking another user's data. Milder is why this seam is
// smaller than bookScoped's, not why it's unnecessary — 23 restatements of
// one idiom is still 23 places that can drift.
