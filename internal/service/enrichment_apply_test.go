...
func applyHarness(t *testing.T) (*EnrichmentService, *fakeBookStore) {
	...
	return NewEnrichmentService(nil, newFakeProviderSettings(), books, &fakeCoverStore{}, nil, writer), books
}
...
func autoEnrichHarness(t *testing.T) (*EnrichmentService, *fakeBookStore) {
	...
	svc := NewEnrichmentService(
		[]provider.Provider{p}, settings, books, &fakeCoverStore{}, nil, writer,
	)
	return svc, books
}
