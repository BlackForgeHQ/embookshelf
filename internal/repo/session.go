...
func (r *SessionRepo) GetActive(ctx context.Context, id string) (model.Session, model.User, error) {
	...
	userRow := r.db.SQL.QueryRowContext(ctx, `SELECT `+userCols+` FROM users WHERE id = $1`, s.UserID)
...
func (r *SessionRepo) Delete(ctx context.Context, id string) error {
	_, err := r.db.SQL.ExecContext(ctx, `DELETE FROM sessions WHERE id = $1`, id)
	return err
}
...
func (r *SessionRepo) PurgeExpired(ctx context.Context) (int64, error) {
	res, err := r.db.SQL.ExecContext(ctx, `DELETE FROM sessions WHERE expires_at <= now()`)
	if err != nil {
		return 0, err
	}
