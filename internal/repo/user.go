...
func (r *UserRepo) GetByID(ctx context.Context, id string) (model.User, error) {
	row := r.db.SQL.QueryRowContext(ctx, `SELECT `+userCols+` FROM users WHERE id = $1`, id)
	return scanUser(row)
}
...
func (r *UserRepo) TouchLastSeen(ctx context.Context, id string, at time.Time) error {
	_, err := r.db.SQL.ExecContext(ctx, `UPDATE users SET last_seen_at = $2 WHERE id = $1`, id, at)
	return err
}
...
func (r *UserRepo) Delete(ctx context.Context, id string) error {
	res, err := r.db.SQL.ExecContext(ctx, `DELETE FROM users WHERE id = $1`, id)
	if err != nil {
		return err
	}
	n, err := res.RowsAffected()
