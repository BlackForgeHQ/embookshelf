// Dashboard with currently reading, activity heatmap, stats

const Dashboard = ({ onOpen }) => {
  ...
  return (
    <div className="fade-in">
      <TopBar title="Good afternoon, Rowan." subtitle={`You've been reading for ${Math.round(totalMin / 60)} hours this quarter across ${readDays} sessions.`} />
      ...
      <div style={{ padding: '28px 32px 80px', display: 'flex', flexDirection: 'column', gap: 40 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
            {reading.map(b => (
              <div key={b.id} onClick={() => onOpen(b.id)} style={{
                ...
                background: 'var(--paper-0)', border: '1px solid var(--rule-soft)',
                ...
                <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  ...
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span className="mono" style={{ fontSize: 10, color: 'var(--ink-3)' }}>{Math.round(b.progress * 100)}%</span>
                      <span className="mono" style={{ fontSize: 10, color: 'var(--ink-3)' }}>p.{Math.round(b.pages * b.progress)}/{b.pages}</span>
                    </div>
                    <div className="progress"><div style={{ width: `${b.progress * 100}%` }} /></div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <section>
          ...
          <div style={{ background: 'var(--paper-0)', border: '1px solid var(--rule-soft)', padding: 24, borderRadius: 2, display: 'flex', gap: 32 }}>
            {/* Heatmap */}
            <div>
              <div style={{ display: 'flex', gap: 4, alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginRight: 6, paddingTop: 2 }}>
                  {['Mon','','Wed','','Fri','',''].map((d,i) => (
                    <div key={i} className="mono" style={{ fontSize: 9, color: 'var(--ink-3)', height: 12 }}>{d}</div>
                  ))}
                </div>
                {weeks.map((week, wi) => (
                  <div key={wi} style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {week.map((m, di) => (
                      <div key={di} title={`${m} min`} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 14 }}>
                ...
                {[0, 15, 30, 45, 60].map(m => <div key={m} style={{ width: 10, height: 10, background: heatColor(m), borderRadius: 1 }} />)}
            ...
            <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 20 }}>
              {[
                ...
                <div key={s.label} style={{ borderLeft: '2px solid var(--accent)', paddingLeft: 14 }}>
                  <div className="t-label" style={{ marginBottom: 6 }}>{s.label}</div>
                  <div style={{ fontFamily: 'Spectral, serif', fontSize: 28, fontWeight: 500, letterSpacing: '-0.01em' }}>{s.value}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 28 }}>
          <section>
            ...
            <div style={{ display: 'flex', gap: 18, overflowX: 'auto', paddingBottom: 8 }}>
              {BOOKS.filter(b => b.shelf?.includes('New')).map(b => (
                <div key={b.id} onClick={() => onOpen(b.id)} style={{ flexShrink: 0, width: 110, cursor: 'pointer' }}>
          ...
          <section>
            <div style={{ marginBottom: 16 }}><h2 className="t-h2">Your libraries</h2></div>
              ...
              {LIBRARIES.map((lib, i) => (
                <div key={lib.id} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div className="mono" style={{ fontSize: 10.5, color: 'var(--ink-3)' }}>{lib.path}</div>
                  </div>
                  <span className="mono" style={{ fontSize: 11, color: 'var(--ink-2)' }}>{lib.count}</span>
                </div>
              ))}
            </div>
          </section>
