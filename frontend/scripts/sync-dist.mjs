// Copies the SPA shell (_shell.html + client assets) from
// frontend/dist/client → internal/staticfs/dist so the Go binary can embed
// it. The SSR server bundle under dist/server is intentionally left behind —
// it exists only for the TanStack Start prerender pass.
import {
  copyFile,
  cp,
  mkdir,
...
const shellRoot = path.join(frontendDir, 'dist', 'client');
const outDist = path.resolve(frontendDir, '..', 'internal', 'staticfs', 'dist');
...
console.log(`synced SPA shell → ${path.relative(frontendDir, outDist)}`);
