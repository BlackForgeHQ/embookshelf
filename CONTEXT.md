...
Since ADR-0035 (#346) the handle is the interface over three **internal seams**, each its own file with its own tests: `libraryKeys` (pure key arithmetic — root, storageKey, walk bases), `bookFiles` (the files-table lookups with one stated absence policy: nil lister answers "no rows"; a failed lookup keeps its error where callers act on it and folds only where advisory) and `bookDelivery` (presign/stream/local). Callers see none of this — the deep-module rule that internal composition is not interface. The nil-orphan-queue degrade stopped being silent at the same time: an object-store `DeleteBookBytes` with no queue answers `ErrNoOrphanQueue` on the bytes-step warning path instead of nil. Do not re-propose exposing or dissolving the handle without new evidence — ADR-0035 records why not.
...
What a PDF's content streams say it is: **TextBased**, **Scanned**, **ImageBased** or **Mixed** — pdf-inspector's sampled answer, taken inside `/convert` before conversion runs, plus the measured verdict after it (ADR-0036). Scanned and ImageBased are refused up front with the per-page evidence — a re-classing, not a new rejection: the engine refuses exactly those verdicts itself, as prose. TextBased and Mixed convert, and the **output is then measured**: a ≥5-page PDF whose markdown lands under a per-page floor is refused as `mixed` (the classifier saw the mixture) or `sparse` (it was fooled — the shape the shipped engine used to serve as 6 silent bytes). Loud on purpose: converting a text-less PDF is how a near-empty markdown silently fed the guide and the generated EPUB. The refusal classes (`scanned`/`image`/`mixed`/`sparse`) travel machine-readably beside `error`, the seam a future OCR stage routes on. pdf-inspector is anydoc's own PDF engine asked the question directly — one parser family, not two. **Distinct** from [[Convertible format]]: that set says which *formats* route to the converter at all; the class says what this *file's bytes* can give once it arrives.
