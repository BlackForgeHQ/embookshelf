...
What a PDF's content streams say it is: **TextBased**, **Scanned** or **Mixed** — pdf-inspector's sampled answer, taken inside `/convert` before conversion runs, plus the measured verdict after it (ADR-0036). TextBased converts; Mixed converts while under half its pages lack a text layer; Scanned is refused with the per-page evidence in the sentence; and a PDF the sampler waved through whose whole conversion lands under a per-page floor is refused as **sparse** — the output is the proof. Loud on purpose: converting a text-less PDF is how a near-empty markdown silently fed the guide and the generated EPUB. The refusal classes (`scanned`/`mixed`/`sparse`) travel machine-readably beside `error`, the seam a future OCR stage routes on. pdf-inspector is anydoc's own PDF engine asked the question directly — one parser family, not two. **Distinct** from [[Convertible format]]: that set says which *formats* route to the converter at all; the class says what this *file's bytes* can give once it arrives.
